...
#define LNAND 24 	// "011000"; -- NAND Rx Ry Rz	-- Rx <- Ry NAND Rz		Format: < inst(6) | Rx(3) | Ry(3) | Rz(3)| x >
...
// funcao usada na logica nand pela ULA
char *converte_int_to_binary_string(int x) {
    char *buffer = (char *) calloc(TAMANHO_PALAVRA, sizeof(char));
    char *buffer1 = (char *) calloc(TAMANHO_PALAVRA, sizeof(char));
    if (x == 0) {
        free(buffer1);
        buffer[0] = '0';
        buffer[1] = '\0';
    ...
    int k = 0;
    for (i = i - 1; k < TAMANHO_PALAVRA && i >= 0; k++, i--) {
        buffer1[k] = buffer[i];
    }
    free(buffer);
    return buffer1;
}
// funcao tambem usada na logica nand pela ULA
int own_strlen(char *buffer) {
    int size = 0;
    while (buffer[size] != '\0')
        size++;
    return size;
...
// tambem usada na logica nand pela ULA
int binary_string_to_int(char *buffer) {
    int x = 0, size = own_strlen(buffer);   
    for (int i = size - 1, k = 0; i >= 0 && k < size; i--, k++) {
        if (buffer[i] == '1') {
            x += pow(2, k);
        }
    }
		...
		case LOGIC:
			if(OP==CMP)
			{
				...
				switch(OP) {
                    ...
                    case LNAND: {
                        result = x & y;
                        // uso de funcoes intermediárias para evitar o comportamento
                        // visto na funcao not anterior. Visto que nand = nand + not:
                        // 1010 nand 1111 => 1010 and 1111 = 1010
                        // => not 1010 = 0101 => resultado = 5
                        char *buffer = converte_int_to_binary_string(result);
                        int size = own_strlen(buffer);
                        ...
                        result = binary_string_to_int(buffer);
                        free(buffer);
                        break;
                    }
					default:
